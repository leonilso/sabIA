  ...
  useEffect(() => {
    async function carregar() {
      try {
        ...
        console.log("o projeto tem os seguintes itens:", data[0])
        setProjetos(data);
      } catch (err) {
        ...
        {projetos.map((projeto) => (
          ...
          >
            <div className="bg-primary rounded-2xl p-4 aspect-[4/3] flex flex-col transition-all duration-300 hover:scale-[1.02] hover:shadow-sabi-lg cursor-pointer relative overflow-hidden">
              ...
              <div className="absolute top-4 left-2 bg-secondary/20 text-secondary-foreground text-[10px] px-2 py-1 rounded-full backdrop-blur-sm font-bold">
            ...
            <Link
              to={`/impressao/${projeto.public_id}`}
              onClick={(e) => e.stopPropagation()}
